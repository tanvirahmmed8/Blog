...
<div class="page-wrapper">
	<div class="container">
        ...
        <?php
        $tag_s = $_GET['tag']; 
         $Pagelimit = 3; // Number of items per page
         $pagea = isset($_GET["page"]) ? (int)$_GET["page"] : 1; // Current page
         $offseta = ($pagea - 1) * $Pagelimit; // Offset for pagination
          $blogs = $db->select('blogs','*',null,"tag LIKE '%$tag_s%' AND deleted_at IS NULL",null,$Pagelimit,$offseta);
        ?>
        ...
        <?php endforeach; ?>
          <?php $db->links('blogs',"tag LIKE '%$tag_s%' AND deleted_at IS NULL") ?>
